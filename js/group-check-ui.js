...
let lastRunG = null;   // most recent successful run, for share links and export
...
function runG() {
  ...
  lastRunG = { v: 1, d: $g("gc-input").value, mode: mode, shape: shape };
...
function buildPythonG() {
  if (!lastRunG) return null;
  const shape = lastRunG.shape;
  const head = `# ${document.title}
# Generated by the Group & Category Check — the same analysis in Python.
...
`;
  if (shape === "distribution") return head + `col = df.select_dtypes("number").iloc[:, 0].dropna()
print(col.describe())
med = col.median()
mad = (col - med).abs().median()
outliers = col[(col - med).abs() > 3.5 * 1.4826 * mad]
print(f"\\noutliers by the MAD rule: {list(outliers.values)}")
jb, jb_p = stats.jarque_bera(col)
print(f"Jarque-Bera p = {jb_p:.4g}  ({'not normal' if jb_p < 0.05 else 'consistent with normal'})")
if mad > 0 and abs(col.mean() - med) / (1.4826 * mad) > 0.5:
    print("mean and median disagree relative to the robust spread — quote the median")
`;
  if (shape === "crosstab") return head + `a, b = df.columns[:2]
table = pd.crosstab(df[a], df[b])
print(table)
chi2, p, dof, expected = stats.chi2_contingency(table, correction=False)
n = table.values.sum()
r, c = table.shape
# bias-corrected Cramér's V (Bergsma) — the uncorrected one flatters small tables
phi2 = max(0.0, chi2 / n - (r - 1) * (c - 1) / (n - 1))
r_adj, c_adj = r - (r - 1) ** 2 / (n - 1), c - (c - 1) ** 2 / (n - 1)
v_corr = np.sqrt(phi2 / max(1e-300, min(r_adj - 1, c_adj - 1)))
print(f"\\nchi2({dof}) = {chi2:.2f}, p = {p:.4g}, corrected Cramér's V = {v_corr:.3f}")
if (expected < 5).mean() > 0.2:
    print("more than 20% of cells expect < 5 — use a permutation test, not this formula")
`;
  // groups: long or wide
  const extract = shape === "groups-long"
    ? `gcol = df.select_dtypes(exclude="number").columns[0]
vcol = df.select_dtypes("number").columns[0]
groups = {k: v[vcol].dropna().values for k, v in df.groupby(gcol)}`
    : `groups = {c: df[c].dropna().values for c in df.select_dtypes("number").columns}`;
  return head + extract + `
names = list(groups)
vals = list(groups.values())
for nm, g in groups.items():
    print(f"{nm:12s} n={len(g):4d}  mean={np.mean(g):8.3f}  median={np.median(g):8.3f}  sd={np.std(g, ddof=1):7.3f}")

if len(vals) == 2:
    a, b = vals
    t, p = stats.ttest_ind(a, b, equal_var=False)          # Welch by default
    print(f"\\nWelch t = {t:.3f}, p = {p:.4g}")
    n1, n2 = len(a), len(b)
    sp = np.sqrt(((n1 - 1) * np.var(a, ddof=1) + (n2 - 1) * np.var(b, ddof=1)) / (n1 + n2 - 2))
    g_es = (np.mean(a) - np.mean(b)) / sp * (1 - 3 / (4 * (n1 + n2) - 9))   # Hedges' g
    print(f"Hedges' g = {g_es:.3f}  (a p-value is not an effect size)")
    u, p_mw = stats.mannwhitneyu(a, b, alternative="two-sided")
    print(f"Mann-Whitney p = {p_mw:.4g}")
else:
    F, p = stats.f_oneway(*vals)
    N, k = sum(map(len, vals)), len(vals)
    grand = np.mean(np.concatenate(vals))
    ssb = sum(len(g) * (np.mean(g) - grand) ** 2 for g in vals)
    ssw = sum(((g - np.mean(g)) ** 2).sum() for g in vals)
    msw = ssw / (N - k)
    omega2 = max(0.0, (ssb - (k - 1) * msw) / (ssb + ssw + msw))
    print(f"\\nANOVA F = {F:.3f}, p = {p:.4g}, omega^2 = {omega2:.3f}")
    H, p_kw = stats.kruskal(*vals)
    print(f"Kruskal-Wallis p = {p_kw:.4g}")
    print(f"note: {k*(k-1)//2} pairwise comparisons would need p < {0.05/(k*(k-1)//2):.4f}, not 0.05")
`;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $g("gc-python").addEventListener("click", () => {
    if (!lastRunG) runG();
    if (!lastRunG) return;
    scShowCode("gc-codebox", buildPythonG());
  });
  scLoadShared((st) => {
    $g("gc-input").value = st.d || "";
    if (st.mode) $g("gc-mode").value = st.mode;
    runG();
  });
