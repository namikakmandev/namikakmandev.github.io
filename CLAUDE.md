...
- `data/` holds curated economics datasets as JSON, indexed by `data/_catalog.json`. Nothing here is edited by hand: `scripts/fetch.py` writes them from `data-sources.json`, and `scripts/build_catalog.py` rebuilds the index. The `.github/workflows/fetch-data.yml` workflow runs every source monthly, the daily and monthly series every morning (`scripts/daily_set.py` picks them), and on dispatch (inputs `only` = space-separated dataset names, `mode` = `discover` to dump a source's raw shape instead of parsing). It commits to `main` itself.
...
- `brief.html` is the front page: `scripts/daily_brief.py` runs after the note in the fetch workflow, sends the note to the Worker's `/v1/ask` in `mode: "brief"`, and keeps the answer as `data/_brief.json` plus a back issue under `data/briefs/`. A failure leaves the previous brief and never blocks the data commit. `js/md.js` renders the markdown; chart links whose series are stored datasets are drawn inline. The BRIEF prompt asks for six or seven stories shaped `### title` / `[chart](plot url)` / caption, and two or three forecasts in the same shape using the `forecast` tool's own chart link (series, dashed path, 95% band); `brief.html` lays each such section out as a grid, the first item spanning both columns when the count is odd. An issue without that shape renders as prose. `econ-mcp.html` also carries a "Brief me now" button (`data-brief` on the ask box in `js/ask.js`): it builds the same context in the browser from `data/_moves.json` and `data/_catalog.json` and sends it to the same mode with an optional `focus`; one brief costs one question of the visitor's allowance.
...
Merging to `main` with changes under `mcp/` triggers Cloudflare Workers Builds (root `mcp`, `npx wrangler deploy`). Dashboard variables survive (`keep_vars`). Bump `SERVER_BUILD` in `mcp/src/version.ts` on every server change; `list_providers` echoes it, which is the only way to confirm a deploy from a client. Variables added in the dashboard create an undeployed version that must be promoted from the Deployments tab. The website's question box (`/v1/ask`, `js/ask.js`) spends the owner's `ANTHROPIC_API_KEY`; `mode: "advisor"` with a `dataset` is the "Explain this dataset" button, a second system prompt that reads one dataset for the visitor; its daily counters are a Durable Object declared in `wrangler.jsonc`, so a change to the class needs a new migration tag.
